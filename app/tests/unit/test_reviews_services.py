...
from core.application.services import reviews_service
...
def test_get_reviews_calls_selectors_list_reviews_with_filters(monkeypatch):
    ...
    monkeypatch.setattr(reviews_service.selectors, "list_reviews", fake_list_reviews)

    out = reviews_service.get_reviews(artist_id="123", album_id="A1")
    assert out == expected
...
def test_get_reviews_calls_selectors_list_reviews_without_filters(monkeypatch):
    ...
    out = reviews_service.get_reviews()
    assert out == expected
